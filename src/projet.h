...
unsigned short shifts[16] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
unsigned short PC1[56] = {
    57, 49, 41, 33, 25, 17, 9,  1,  58, 50, 42, 34, 26, 18, 10, 2,  59, 51, 43,
    35, 27, 19, 11, 3,  60, 52, 44, 36, 63, 55, 47, 39, 31, 23, 15, 7,  62, 54,
    46, 38, 30, 22, 14, 6,  61, 53, 45, 37, 29, 21, 13, 5,  28, 20, 12, 4};

unsigned short PC2[56] = {14, 17, 11, 24, 1,  5,  3,  28, 15, 6,  21, 10,
...
INT Sb[8][64] = {{14, 4,  13, 1, 2,  15, 11, 8,  3,  10, 6,  12, 5,  9,  0, 7,
                 ...
                 {10, 0, 9, 14, 6, 3, 15, 5,  1, 13, 12, 7,  11, 4,  2, 8,  13,
...
unsigned short IP[64] = {58, 50, 42, 34, 26, 18, 10, 2,  60, 52, 44, 36, 28,
...
unsigned short RFP[64] = {40, 8,  48, 16, 56, 24, 64, 32, 39, 7,  47, 15, 55,
                          23, 63, 31, 38, 6,  46, 14, 54, 22, 62, 30, 37, 5,
                          45, 13, 53, 21, 61, 29, 36, 4,  44, 12, 52, 20, 60,
                          28, 35, 3,  43, 11, 51, 19, 59, 27, 34, 2,  42, 10,
                          50, 18, 58, 26, 33, 1,  41, 9,  49, 17, 57, 25};

unsigned short E[48] = {32, 1,  2,  3,  4,  5,  4,  5,  6,  7,  8,  9,
                        8,  9,  10, 11, 12, 13, 12, 13, 14, 15, 16, 17,
                        16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25,
                        24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1};

unsigned short P[32] = {16, 7, 20, 21, 29, 12, 28, 17, 1,  15, 23,
                        26, 5, 18, 31, 10, 2,  8,  24, 14, 32, 27,
                        3,  9, 19, 13, 30, 6,  22, 11, 4,  25};


// tableau pour la cryptanalyse différentielle (cf exo 6)
static int J[8][64] = {0};
//==================================
// PROTOTYPES
//==================================
...
// void reverse_key_schedule(int rounds, BYTE KR[48], BYTE kSortie[64]);
// ancienne version mais Kr ne servait pas
void reverse_key_schedule(int rounds, BYTE kSortie[64]);

void DES(BYTE *in, BYTE *out, int rounds, BYTE (*KS)[48]);

/* Encryption/decryption
 * The number of rounds should preferably be greater or equal to 16 in order to
 * withstand known-plaintext attacks. As explained by Schneier's description of
 * DES in Chapter 12 of Applied Cryptography (12.3): `DES with any number of
 * rounds fewer than 16 could be broken with a known-plaintext attack more
 * efficiently than by a brute-force attack.` (see
 * https://crypto.stackexchange.com/questions/9678/why-does-des-use-exactly-16-rounds).
 */
